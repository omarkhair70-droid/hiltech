...
Phase 2 — Shared Product Infrastructure — is ACTIVE. Slices 01–05 are VERIFIED and merged. Slice 05 — Inbox / Work Queue Foundation — merged as `6b6194a9107dcfc1ba74e392776a1dee9e38964a`; post-merge Bootstrap run `35468123129` is **PASS** with foundation, database, local-platform, evidence-storage, Terraform and supply-chain gates green. Slice 05 adds V0013 durable Inbox/read-state persistence, Approval-driven source-linked projection, current source/authority/OpenFGA revalidation, per-user idempotent read/unread, deterministic Work Queue/Inbox pagination, one shared Android/Windows client, and durable Spring Modulith recovery without duplicate effect. Source domains remain authoritative. The current target is **Slice 06 — Notification Abstraction**: provider-neutral delivery intent/policy/attempt foundations only; no push/email/SMS provider, user preferences, quiet-hours, digest or escalation policy may be invented.
...
3. The current shared capability in the frozen Phase 2 order is **Notification Abstraction**. Use the Slice 06 reality/implementation contracts; do not infer production provider, channel enablement, preferences, quiet hours, digest or escalation policy from Inbox.
